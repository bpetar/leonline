 ...
 * icons of items placed in container. This class has array of icon objects defined in 
 * class CPickableItem. CGUIContainer can be added to GUI window like any other irrlicht 
...
/**
 * \brief Returns CPickableItem in position (x,y).
 * \author Petar Bajic 
 * \date July, 21 2008.
 */
...
/**
 * \brief Returns CPickableItem at array[index].
 * \author Petar Bajic 
 * \date July, 21 2008.
 */
CGameObject* CGUIContainer::GetItem(u32 index)
{
	return m_ListOfItems[index].pick;
}

/**
 * \brief Sets CPickableItem in position (x,y).
 * \author Petar Bajic 
 * \date July, 21 2008.
 */
void CGUIContainer::SetItem(u32 x, u32 y, CGameObject* pick)
{
	...
	{
		//TODO: do something with existing item (switching?), or ignore setting new one
		if (m_Overwrite)
		{
...
/**
 * \brief Insert CPickableItem to first available position.
 * \author Petar Bajic 
 * \date July, 21 2008.
 */
...
/**
 * \brief Clears the array of CPickableItem objects - m_ListOfItems.
 * \author Petar Bajic 
 * \date July, 21 2008.
 */
				...
				case EMIE_LMOUSE_PRESSED_DOWN:
					{
						for (u32 x = 0; x < m_Width; x++)
						{
							for (u32 y = 0; y < m_Height; y++)
							{
								if (isInsideSlot(x,y,p))
								{
									//Simple mouse click is handled like request to remove item.
									//Advanced implementation would involve selection of item
									//Then pressing del key would remove item (or dragging it away from component)
									RemoveItem(x,y); 
								}
							}
						}
