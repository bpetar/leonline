...
* Code is changed a little here. It is dialog for saving map files.
...
CGUIFileSaveDialog::CGUIFileSaveDialog(const wchar_t* title, 
		IGUIEnvironment* environment, IGUIElement* parent, s32 id, const wchar_t* ext)
  ...
  Dragging(false), FileNameText(0), FileList(0)
{
   ...
   fillListBox();
	...
	Extension = ext;
	stringw untitled_filename = stringw("untitled.") + Extension;
...
void CGUIFileSaveDialog::fillListBox()
{
   ...
   core::stringw s;
   core::stringw ext = stringw(".") + Extension;

   for (u32 i=0; i<FileList->getFileCount(); ++i)
   {
      s = FileList->getFileName(i);
	  if (FileList->isDirectory(i) || (s.find(ext.c_str()) > -1))
...
{
	//Automatic adding irr extension if not existing
